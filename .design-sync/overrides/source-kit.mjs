...
// forked from design-sync lib/source-kit.mjs — record the src file of each
// synth-derived component so its group (@category JSDoc) resolves even for
// secondary exports of multi-export files (e.g. CardHeader in card.tsx, the
// Typography scale in Typography.tsx). Without this, deriveComponentsFromSrc
// discards the file and enrichment's name-based fuzzy-find can't relocate it,
// stranding those components in the 'general' group.
...
function deriveComponentsFromSrc(srcFiles) {
  ...
  const seen = new Map(); // name -> first src file it was exported from
...
export async function resolvePackage(ctx) {
  ...
  if (srcRoot) {
    for (const c of components) {
      // Pinned via config → skip fuzzy-find. Else use the file deriveComponents
      // recorded (handles secondary exports), and only then fall to fuzzy-find.
